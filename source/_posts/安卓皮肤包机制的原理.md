...
一种常用的方案就是将使用到的字符串、图片等资源打包到一个皮肤apk中，这个皮肤apk中国只包含资源没有任何的代码。主apk在启动之后从这个皮肤apk中加载资源，从而减少主apk的大小，同时也将不同皮肤的资源集合在一起提高可维护性。

# 获取皮肤apk的Resources

我们都知道安卓应用的资源都是通过Resources去管理的,只要能获取到皮肤apk的Resources,那么就能够读取到皮肤apk中的资源文件。

## 方法一

第一种获取Resource的方式是通过皮肤apk的Context去获取:
...
这种方式很简单,但是却有一定的局限性。因为要使用这种方式去获取Reources,必须将皮肤apk也安装到系统。

## 方法二

第二种方法是通过获取皮肤apk的AssetManager,直接new一个Recources出来:
...
这种方向需要用到反射去调用AssetManager.addAssetPath,但是只需要将皮肤apk下载到系统,并不需要安装也能读取到里面的资源。

# 加载资源

一般我们都是通过资源的id从Resources中加载资源的,但是当资源在皮肤apk里面的时候,我们没有办法直接通过R类知道资源的id,所以需要使用Resources的getIdentifier方法去获取id:
